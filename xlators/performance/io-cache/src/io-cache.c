        ...
        while (trav_offset < rounded_end) {
                ioc_inode_lock (ioc_inode);
                {
						...
						syslog(LOG_INFO | LOG_LOCAL1,
							   "%s (%lu)", __func__, (unsigned long) trav_size);
                        ...
                        if (!trav) {
								...
								syslog(LOG_INFO | LOG_LOCAL1, "%s[%d]", __func__, __LINE__);
...
init (xlator_t *this)
{
		...
		syslog(LOG_INFO | LOG_LOCAL0,
			   "%s parent name : %s",
			   this->name, this->parents->xlator->name);
