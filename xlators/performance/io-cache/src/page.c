               ...
               dict_t *xdata)
{
		...
		syslog(LOG_INFO | LOG_LOCAL0,
			   "%s (%lu)",
			   __func__, (unsigned long) vector[0].iov_len);
                ...
                if (op_ret < 0) {
                        ...
                        if (!page) {
								...
								syslog(LOG_INFO | LOG_LOCAL1,
									   "%s iobref_page_size(%lu)",
									   __func__, (unsigned long) iobref_page_size);
#endif
                ...
                off_t offset)
{
        ...
        fault_local->pending_size = table->page_size; /* dskim! */
        fault_local->inode = ioc_inode;

#if 0
		syslog(LOG_INFO | LOG_LOCAL1,
			   "%s (%lu)",
			   __func__, (unsigned long) table->page_size);
#endif
                  ...
                  size_t size, int32_t op_errno)
{
        ...
        if (local->op_ret != -1) {
...

                {
						...
						syslog(LOG_INFO | LOG_LOCAL1,
							   "%s size(%lu)", __func__, (unsigned long) copy_size);
...
__ioc_page_wakeup (ioc_page_t *page, int32_t op_errno)
{
        ...
        for (trav = waitq; trav; trav = trav->next) {
                frame = trav->data;
				syslog(LOG_INFO | LOG_LOCAL1, "%s (%lu)", __func__, (unsigned long) trav->pending_size);
