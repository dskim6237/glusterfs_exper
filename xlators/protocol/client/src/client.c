              ...
              off_t offset, uint32_t flags, dict_t *xdata)
{
		...
		syslog(LOG_INFO | LOG_LOCAL0,
			   "%s (%lu)",
			   __func__, (unsigned long)args.size);
...
init (xlator_t *this)
{
        ...
        */
		...
		syslog(LOG_INFO | LOG_LOCAL0,
			   "%s prev(%s) next(%s) parent(%s)",
			   this->name, this->prev != NULL ? this->prev->name : "null", this->next != NULL ? this->next->name : "null", this->parents->xlator->name);

		syslog(LOG_INFO | LOG_LOCAL0, "--");
		temp = this;
		while (temp != NULL) {
			syslog(LOG_INFO | LOG_LOCAL0, "%s", temp->name);
			temp = temp->prev;
		}
