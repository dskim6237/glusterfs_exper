...
pkgdatadir = $(datadir)/glusterfs
pkgincludedir = $(includedir)/glusterfs
pkglibdir = $(libdir)/glusterfs
pkglibexecdir = $(libexecdir)/glusterfs
...
DIST_COMMON = README $(am__configure_deps) $(srcdir)/Makefile.am \
	...
	$(srcdir)/glusterfs-api.pc.in $(srcdir)/glusterfs.spec.in \
	$(srcdir)/libgfchangelog.pc.in $(top_srcdir)/configure \
	$(top_srcdir)/contrib/uuid/uuid_types.h.in AUTHORS ChangeLog \
	INSTALL NEWS THANKS config.guess config.sub depcomp install-sh \
...
CONFIG_CLEAN_FILES = contrib/uuid/uuid_types.h glusterfs-api.pc \
	libgfchangelog.pc glusterfs.spec
...
ACLOCAL = ${SHELL} /root/glusterfs-3.5.2/missing --run aclocal-1.11
AMTAR = ${SHELL} /root/glusterfs-3.5.2/missing --run tar
...
AUTOCONF = ${SHELL} /root/glusterfs-3.5.2/missing --run autoconf
AUTOHEADER = ${SHELL} /root/glusterfs-3.5.2/missing --run autoheader
AUTOMAKE = ${SHELL} /root/glusterfs-3.5.2/missing --run automake-1.11
AWK = gawk
BUILD_PYTHON_INC = /usr/include/python2.7
BUILD_PYTHON_LIB = python2.7
CC = gcc
CCDEPMODE = depmode=gcc3
CFLAGS = -g -O2 -Wformat -Werror=format-security -Werror=implicit-function-declaration -I/usr/include/python2.7 -I/usr/include/python2.7 -fno-strict-aliasing -O2 -g -pipe -Wall -Wp,-D_FORTIFY_SOURCE=2 -fexceptions -fstack-protector-strong --param=ssp-buffer-size=4 -grecord-gcc-switches -m64 -mtune=generic -D_GNU_SOURCE -fPIC -fwrapv -DNDEBUG -O2 -g -pipe -Wall -Wp,-D_FORTIFY_SOURCE=2 -fexceptions -fstack-protector-strong --param=ssp-buffer-size=4 -grecord-gcc-switches -m64 -mtune=generic -D_GNU_SOURCE -fPIC -fwrapv
...
LDFLAGS =  -L/usr/lib -lpthread -ldl -lutil -lm -lpython2.7 -Xlinker -export-dynamic
LEX = flex
LEXLIB = 
...
LN_S = ln -s
LTLIBOBJS = 
MAKEINFO = ${SHELL} /root/glusterfs-3.5.2/missing --run makeinfo
...
PACKAGE_STRING = glusterfs 3.5.2
...
ZLIB_CFLAGS =  
ZLIB_LIBS = -lz  
abs_builddir = /root/glusterfs-3.5.2
abs_srcdir = /root/glusterfs-3.5.2
abs_top_builddir = /root/glusterfs-3.5.2
abs_top_srcdir = /root/glusterfs-3.5.2
ac_ct_CC = gcc
ac_ct_DUMPBIN = 
am__include = include
am__leading_dot = .
...
initdir = /etc/init.d
install_sh = ${SHELL} /root/glusterfs-3.5.2/install-sh
...
SUBDIRS = argp-standalone libglusterfs rpc api xlators glusterfsd \
	$(FUSERMOUNT_SUBDIR) doc extras cli heal geo-replication
...
contrib/uuid/uuid_types.h: $(top_builddir)/config.status $(top_srcdir)/contrib/uuid/uuid_types.h.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
glusterfs-api.pc: $(top_builddir)/config.status $(srcdir)/glusterfs-api.pc.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
libgfchangelog.pc: $(top_builddir)/config.status $(srcdir)/libgfchangelog.pc.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
glusterfs.spec: $(top_builddir)/config.status $(srcdir)/glusterfs.spec.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
